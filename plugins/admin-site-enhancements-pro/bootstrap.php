	...
	private function __construct() {
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'custom_content_types', $options ) && $options['custom_content_types'] ) {
				...
				add_action( 'manage_asenha_ctax_posts_columns', [ $content_management, 'ctax_posts_define_columns__premium_only' ] );
		...
		if ( array_key_exists( 'content_order', $options ) && $options['content_order'] ) {
			if ( array_key_exists( 'content_order_for', $options ) && ! empty( $options['content_order_for'] ) )  {
				...
				add_filter( 'save_post', [ $content_management, 'set_menu_order_for_new_posts' ], 10, 3 );
			}
		...
		if ( array_key_exists( 'enable_media_replacement', $options ) && $options['enable_media_replacement'] ) {
			...
			add_filter('wp_calculate_image_srcset', [ $content_management, 'append_cache_busting_param_to_image_srcset' ], 10, 5 );
			add_filter('wp_get_attachment_image_src', [ $content_management, 'append_cache_busting_param_to_attachment_image_src' ], 10, 2 );
			add_filter('wp_prepare_attachment_for_js', [ $content_management, 'append_cache_busting_param_to_attachment_for_js' ], 10, 2 );
			add_filter('wp_get_attachment_url', [ $content_management, 'append_cache_busting_param_to_attachment_url' ], 20, 2 );
		}
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'enable_avif_upload', $options ) && $options['enable_avif_upload'] ) {
				add_filter( 'mime_types', [ $content_management, 'add_avif_mime_type__premium_only' ] );
				add_filter( 'upload_mimes', [ $content_management, 'allow_avif_mime_type_upload__premium_only' ] );
				add_filter( 'getimagesize_mimes_to_exts', [ $content_management, 'add_avif_mime_type_to_exts__premium_only' ] );
				add_filter( 'wp_generate_attachment_metadata', [ $content_management, 'add_avif_image_dimension__premium_only' ], 10, 3 );
				add_filter( 'file_is_displayable_image', [ $content_management, 'make_avif_displayable__premium_only' ], 10, 2 );
				add_filter( 'wp_check_filetype_and_ext', [ $content_management, 'handle_exif_and_fileinfo_fail__premium_only' ], 10, 5 );
			}			
		}
		...
		if ( array_key_exists( 'custom_nav_menu_items_new_tab', $options ) && $options['custom_nav_menu_items_new_tab'] ) {
			add_filter( 'wp_nav_menu_item_custom_fields', [ $content_management,'add_custom_nav_menu_open_in_new_tab_field'], 10, 4 );
			add_action( 'wp_update_nav_menu_item', [ $content_management, 'save_custom_nav_menu_open_in_new_tab_status'], 10, 3 );
			add_action( 'nav_menu_link_attributes', [ $content_management, 'add_attributes_to_custom_nav_menu_item' ], 10, 3 );
		}
		...
		if ( array_key_exists( 'hide_modify_elements', $options ) && $options['hide_modify_elements'] ) {
	        ...
	        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
				...
				add_action( 'wp_before_admin_bar_render', [ $admin_interface, 'capture_extra_admin_bar_nodes__premium_only' ], 99999 );
				// Remove admin bar nodes from plugins
				add_filter( 'wp_before_admin_bar_render', [ $admin_interface, 'remove_extra_admin_bar_nodes__premium_only' ], PHP_INT_MAX );
			}
		}
		...
		if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'hide_admin_bar', $options ) && $options['hide_admin_bar'] && array_key_exists( 'hide_admin_bar_on_backend_for', $options ) && isset( $options['hide_admin_bar_on_backend_for'] ) ) {
				add_action( 'admin_print_scripts', [ $admin_interface, 'hide_admin_bar_for_roles_on_backend__premium_only' ] );
			}			
		}
		...
		if ( array_key_exists( 'customize_admin_menu', $options ) && $options['customize_admin_menu'] ) {
			...
			if ( array_key_exists( 'custom_menu_hidden', $options ) 
		        ...
		        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
		        	add_action( 'current_screen', [ $admin_interface, 'maybe_restrict_access_to_current_screen__premium_only' ] );
		        }
			}
	        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
				add_action( 'wp_ajax_reset_admin_menu', [ $admin_interface, 'reset_admin_menu__premium_only' ] );
			}
		}
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'admin_columns_manager', $options ) && $options['admin_columns_manager'] ) {
				add_action( 'admin_menu', [ $admin_interface, 'add_admin_columns_organizer_menu__premium_only' ] );
				// add_action( 'admin_init', [ $admin_interface, 'register_admin_columns_sections_fields__premium_only' ] );

				add_action( 'admin_footer', [ $admin_interface, 'add_additional_elements__premium_only' ] );
				add_filter( 'admin_enqueue_scripts', [ $admin_interface, 'add_list_tables_scripts__premium_only' ] );

				// add_action( 'wp_loaded', [ $admin_interface, 'render_columns__premium_only' ] );
				add_action( 'admin_init', [ $admin_interface, 'render_columns__premium_only' ] );
				add_action( 'wp_ajax_save_columns_order', [ $admin_interface, 'save_columns_order__premium_only' ] );
				add_action( 'wp_ajax_reset_columns', [ $admin_interface, 'reset_columns__premium_only' ] );
				add_action( 'wp_ajax_reindex_custom_fields', [ $admin_interface, 'reindex_custom_fields__premium_only' ] );

				// For implementing custom styles to list tables
				add_filter( 'admin_body_class', [ $admin_interface, 'admin_columns_body_class__premium_only'], PHP_INT_MAX );
				// add_filter( 'admin_enqueue_scripts', [ $admin_interface, 'admin_columns_css__premium_only' ] );
				add_filter( 'admin_print_footer_scripts', [ $admin_interface, 'admin_columns_css__premium_only' ], PHP_INT_MAX );
				...
				add_action( 'wp_ajax_inline-save', [ $admin_interface, 'wp_ajax_inline_save_with_page_reload' ], 0 );
			}
        }
			...
			if ( array_key_exists( 'login_id_type', $options ) && ! empty( $options['login_id_type'] ) )  {
				switch ( $options['login_id_type'] ) {
					case 'username':
						...
						add_filter( 'gettext', [ $login_logout, 'gettext_login_id_username' ], 20, 3 );					
						break;
					case 'email';
						add_filter( 'authenticate', [ $login_logout, 'authenticate_email' ], 20, 2 );
						add_filter( 'gettext', [ $login_logout, 'gettext_login_id_email' ], 20, 3 );					
						break;
				}
			}
		}
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'enable_code_snippets_manager', $options ) && $options['enable_code_snippets_manager'] ) {
				...
				add_action( 'pre_post_update', [ $custom_code, 'record_last_edited_php_snippets__premium_only' ], 10, 2 );
				add_action( 'plugins_loaded', [ $custom_code, 'maybe_show_safe_mode_admin_bar_icon__premium_only' ] );
				...
				if ( false === strpos( wp_login_url(), $_SERVER['SCRIPT_NAME'] )
					...
					add_filter( 'wp_die_handler', [ $custom_code, 'custom_wp_die_handler__premium_only' ] );					
				}
			}
        }
		...
		if ( array_key_exists( 'enable_custom_frontend_css', $options ) && $options['enable_custom_frontend_css'] ) {
	        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
				if ( array_key_exists( 'custom_frontend_css_priority', $options ) ) {
					add_filter( 'wp_head', [ $custom_code, 'custom_frontend_css' ], $options['custom_frontend_css_priority'] );
				} else {
		...
		if ( array_key_exists( 'disable_comments', $options ) && $options['disable_comments'] ) {
			if ( array_key_exists( 'disable_comments_for', $options ) && ! empty( $options['disable_comments_for'] ) )  {
				add_action( 'admin_init', [ $disable_components, 'disable_comments_for_post_types_edit' ] ); // also work with 'init', 'admin_init', 'wp_loaded', 'do_meta_boxes' hooks
				...
				add_filter( 'comments_array', [ $disable_components, 'maybe_return_empty_comments' ], 20, 2 );
				add_filter( 'comments_open', [ $disable_components, 'close_comments_pings_on_frontend' ], 20, 2 );
				add_filter( 'pings_open', [ $disable_components, 'close_comments_pings_on_frontend' ], 20, 2 );
				add_filter( 'get_comments_number', [ $disable_components, 'return_zero_comments_count' ], 20, 2 );
				...
				add_filter( 'rest_pre_insert_comment', [ $disable_components, 'return_blank_comment' ], 10, 2 );
			}
		}
		...
		if ( array_key_exists( 'disable_feeds', $options ) && $options['disable_feeds'] ) {
			...
			add_action( 'template_redirect', [ $disable_components, 'redirect_feed_to_403' ], 10, 1 );
		}
        ...
        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
			...
			if ( array_key_exists( 'local_user_avatar', $options ) && $options['local_user_avatar'] ) {
				...
				add_action( 'show_user_profile', [ $utilities, 'render_personal_profile_fields__premium_only' ] );
				// For when admins edit user profiles
				add_action( 'edit_user_profile', [ $utilities, 'render_personal_profile_fields__premium_only' ] );
				
				// Update Profile Picture fields / user meta
				// For when current user edits their own profile
				add_action( 'personal_options_update', [ $utilities, 'update_personal_profile_user_meta__premium_only' ] );
				// For when admins edit user profiles
				add_action( 'edit_user_profile_update', [ $utilities, 'update_personal_profile_user_meta__premium_only' ] );
				
				// Delete Profile Picture user meta when attachment is deleted
				add_action( 'delete_attachment', [ $utilities, 'delete_personal_profile_user_meta__premium_only' ] );
				
				// Override avatar with the local one uploaded
				add_filter( 'get_avatar', [ $utilities, 'override_avatar_with_local__premium_only' ], 5, 5 );
				add_filter( 'get_avatar_url', [ $utilities, 'override_avatar_url_with_local__premium_only' ], 10, 3 );	
			}
        }
