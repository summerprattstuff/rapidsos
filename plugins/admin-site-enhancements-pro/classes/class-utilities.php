...
class Utilities {
	...
	public function render_personal_profile_fields__premium_only( $user ) {
		...
		<input type="hidden" name="local_user_avatar_attachment_id" value="<?php echo $attachment_id; ?>" />
		<?php
	}
	...
	public function override_avatar_url_with_local__premium_only( $url, $id_or_email, $args ) {
		...
		// return 'https://wpplugins-build.dev.bowo.io/wp-content/uploads/2023/12/wexor-tmg-L-2p8fapOA8-unsplash-150x150.webp';
	}
			...
			foreach ( $image_sizes as $size ) {
				...
				if ( in_array( $size, $generated_sizes ) || 'full' == $size ) {
					...
					if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
						echo '<td class="copy"><span class="copy-url-button" data-clipboard-text="'. $src[0] .'"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24"><g fill="none" stroke="#8C8F94" stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"><path d="M16 3H4v13"/><path d="M8 7h12v12a2 2 0 0 1-2 2h-8a2 2 0 0 1-2-2z"/></g></svg></span><span class="url-copied" style="display:none;">URL copied!</span></td>';
					}
		...
		if ( ! in_array( 'wp-admin', $current_url_parts ) || ( false !== strpos( 'wp-login.php', $current_url ) ) ) {
			if ( ! current_user_can( 'manage_options' ) ) {
				...
				if ( 'lines' === $background ) { // https://bgjar.com/curve-line
			        ...
			        if ( bwasenha_fs()->can_use_premium_code__premium_only() ) {
						$background_image = ( isset( $options['maintenance_page_background_image'] ) ) ? str_replace( get_site_url() . '/', '', $options['maintenance_page_background_image'] ) : '';
